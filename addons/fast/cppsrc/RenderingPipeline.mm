...
static NSString *const kShaderSource = MTL_STRINGIFY(
    ...
    vertex ProjectedVertex vertex_shader(InputVertex input_vertex
                                         [[stage_in]]) {
    ...
    fragment half4 fragment_shader(
      ...
      float2 uv =
          chroma_texture.sample(frame_sampler, input.uv).rg - float2(0.5, 0.5);

      // Conversion for YUV to rgb from http://www.fourcc.org/fccyvrgb.php
      float4 out = float4(y + 1.403 * uv.y, y - 0.344 * uv.x - 0.714 * uv.y,
                          y + 1.770 * uv.x, 1.0);
...
- (instancetype)initWithLayer:(CAMetalLayer *)layer error:(NSError **)error {
  ...
  if (self) {
    ...
    id<MTLFunction> fragmentFunction =
        [library newFunctionWithName:@"fragment_shader"];
    ...
    vertexDescriptor.layouts[0].stepFunction =
        (MTLVertexStepFunction)MTLStepFunctionPerVertex;
    ...
    _state = [_device newRenderPipelineStateWithDescriptor:pipelineDescriptor
                                                     error:error];
    if (_state == nil) {
      return nil;
    }
    ...
    _vertexBuffer = [_device newBufferWithLength:16 * sizeof(simd_float1)
                                         options:0];

    if (CVMetalTextureCacheCreate(kCFAllocatorDefault, NULL, _device, NULL,
...
- (BOOL)setupTexturesForFrame:(CVPixelBufferRef)frame {
  ...
  CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, _textureCache,
                                            frame, NULL, MTLPixelFormatR8Unorm,
  ...
  CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, _textureCache,
                                            frame, NULL, MTLPixelFormatRG8Unorm,
                                            width / 2, height / 2, 1, &texture);
  if (texture == NULL) {
    return NO;
  }
...
- (void)setupEncoder:(id<MTLRenderCommandEncoder>)commandEncoder
            forFrame:(CVPixelBufferRef)frame {
  int width = (int)CVPixelBufferGetWidth(frame);
  int height = (int)CVPixelBufferGetHeight(frame);

  // Set up the quad vertices with respect to the orientation and aspect ratio
  // of the video.
  CGSize aspectRatio = CGSizeMake(width, height);
  CGRect vertexSamplingRect =
      AVMakeRectWithAspectRatioInsideRect(aspectRatio, _layer.bounds);

  // Compute normalized quad coordinates to draw the frame into.
  CGSize normalizedSamplingSize = CGSizeMake(0.0, 0.0);
  CGSize cropScaleAmount =
      CGSizeMake(vertexSamplingRect.size.width / _layer.bounds.size.width,
  ...
  simd_float1 vertexData[] = {x1, y1, u1, v1, x2, y2, u2, v2,
