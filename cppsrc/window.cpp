 ...
 *
 * Valuable Resources:
 * http://ffmpeg.org/doxygen/trunk/ffmpeg__videotoolbox_8c_source.html (Provides some insight into decoded frames)
 * https://medium.com/liveop-x-team/accelerating-h264-decoding-on-ios-with-ffmpeg-and-videotoolbox-1f000cb6c549
 * https://ffmpeg.org/doxygen/3.4/hw_decode_8c-example.html (shows a hardware decode example in c)
 * https://stackoverflow.com/questions/21007329/what-is-an-sdl-renderer (helpful SDL terminology)
 *
 */
...
/**
 * If user does not have hardware accelerated capability, fall back to software format.
 */
static enum AVPixelFormat get_hw_surface_fmt(struct AVCodecContext *s, const enum AVPixelFormat *fmt) {
    while (*fmt != AV_PIX_FMT_NONE) {
        if (*fmt == AV_PIX_FMT_VIDEOTOOLBOX) {
            if (s->hwaccel_context == NULL) {
                ...
                if (result < 0) {
                    std::cerr << "Hardware decoder failed to initialize, falling back to " << s->pix_fmt << std::endl;
                    return s->pix_fmt;
                }
    ...
    std::cerr << "Did not find Videotoolbox, falling back to format " << s->pix_fmt << std::endl;
    return s->pix_fmt;
}
...
int run_program(std::string filename) {
    ...
    if (avformat_open_input(&pFormatCtx, filename.c_str(), nullptr /* autodetect format */, nullptr) != 0) {
    ...
    ret = av_find_best_stream(pFormatCtx, AVMEDIA_TYPE_VIDEO, -1, -1, &decoder, 0);
    if (ret < 0) {
    ...
    if (!(decoderCtx = avcodec_alloc_context3(decoder)))
    ...
    // get_hw_surface_fmt will be called with all available with the AVCodecContext and all
    // available formats for the codec.
    ...
    yPlaneSz = decoderCtx->width * decoderCtx->height;
    uvPlaneSz = decoderCtx->width * decoderCtx->height / 4;
        ...
        if (packet->stream_index == videoStream) {
            ...
            if (result == 0) {
                ...
                if (is_hw_decoded(frame)) {
                    ...
                    // std::cout << "Format of CVPixelBuffer: " << (type == kCVPixelFormatType_420YpCbCr8Planar) << ", frame format: " << AV_PIX_FMT_GBRAP10LE << std::endl;
                    ...
                    SDL_UpdateYUVTexture(
                            ...
                            nullptr,
                            yDestPlane,
                            CVPixelBufferGetBytesPerRowOfPlane(ref, 0),
                            uDestPlane,
                            CVPixelBufferGetBytesPerRowOfPlane(ref, 1),
                            vDestPlane,
                    ...
                    pict.data[1] = uPlane;
                    pict.data[2] = vPlane;
                    pict.linesize[0] = decoderCtx->width;
                ...
                std::cerr << "Output is not available in this state - send more input to decoder." << std::endl;
